// Generated Dart file from ky.json
// Do not edit manually

Map<String, Map<String, String>> get kyJson => {
  'KY': {
    'fmt': '%N%n%O%n%A%n%S %Z',
    'id': 'data/KY',
    'key': 'KY',
    'lang': 'en',
    'languages': 'en',
    'name': 'CAYMAN ISLANDS',
    'posturl': 'http://www.caymanpost.gov.ky/',
    'require': 'AS',
    'state_name_type': 'island',
    'sub_keys': 'Cayman Brac~Grand Cayman~Little Cayman',
    'zip': 'KY\\d-\\d{4}',
    'zipex': 'KY1-1100,KY1-1702,KY2-2101',
  },
  'KY/Cayman Brac': {'id': 'data/KY/Cayman Brac', 'key': 'Cayman Brac', 'lang': 'en'},
  'KY/Grand Cayman': {'id': 'data/KY/Grand Cayman', 'key': 'Grand Cayman', 'lang': 'en'},
  'KY/Little Cayman': {
    'id': 'data/KY/Little Cayman',
    'key': 'Little Cayman',
    'lang': 'en',
  },
};
