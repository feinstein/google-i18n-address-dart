// Generated Dart file from so.json
// Do not edit manually

Map<String, Map<String, String>> get soJson => {
  'SO': {
    'fmt': '%N%n%O%n%A%n%C, %S %Z',
    'id': 'data/SO',
    'key': 'SO',
    'lang': 'so',
    'languages': 'so',
    'name': 'SOMALIA',
    'require': 'ACS',
    'sub_isoids': 'AW~BK~BN~BR~BY~GA~GE~HI~JD~JH~MU~NU~SA~SD~SH~SO~TO~WO',
    'sub_keys': 'AD~BK~BN~BR~BY~GG~GD~HR~JD~JH~MD~NG~SG~SD~SH~SL~TG~WG',
    'sub_names':
        'Awdal~Bakool~Banaadir~Bari~Bay~Galguduud~Gedo~Hiiraan~Jubbada Dhexe~Jubbada Hoose~Mudug~Nugaal~Sanaag~Shabeellaha Dhexe~Shabeellaha Hoose~Sool~Togdheer~Woqooyi Galbeed',
    'upper': 'ACS',
    'zip': '[A-Z]{2} ?\\d{5}',
    'zipex': 'JH 09010,AD 11010',
  },
  'SO/AD': {
    'id': 'data/SO/AD',
    'isoid': 'AW',
    'key': 'AD',
    'lang': 'so',
    'name': 'Awdal',
  },
  'SO/BK': {
    'id': 'data/SO/BK',
    'isoid': 'BK',
    'key': 'BK',
    'lang': 'so',
    'name': 'Bakool',
  },
  'SO/BN': {
    'id': 'data/SO/BN',
    'isoid': 'BN',
    'key': 'BN',
    'lang': 'so',
    'name': 'Banaadir',
  },
  'SO/BR': {'id': 'data/SO/BR', 'isoid': 'BR', 'key': 'BR', 'lang': 'so', 'name': 'Bari'},
  'SO/BY': {'id': 'data/SO/BY', 'isoid': 'BY', 'key': 'BY', 'lang': 'so', 'name': 'Bay'},
  'SO/GD': {'id': 'data/SO/GD', 'isoid': 'GE', 'key': 'GD', 'lang': 'so', 'name': 'Gedo'},
  'SO/GG': {
    'id': 'data/SO/GG',
    'isoid': 'GA',
    'key': 'GG',
    'lang': 'so',
    'name': 'Galguduud',
  },
  'SO/HR': {
    'id': 'data/SO/HR',
    'isoid': 'HI',
    'key': 'HR',
    'lang': 'so',
    'name': 'Hiiraan',
  },
  'SO/JD': {
    'id': 'data/SO/JD',
    'isoid': 'JD',
    'key': 'JD',
    'lang': 'so',
    'name': 'Jubbada Dhexe',
  },
  'SO/JH': {
    'id': 'data/SO/JH',
    'isoid': 'JH',
    'key': 'JH',
    'lang': 'so',
    'name': 'Jubbada Hoose',
  },
  'SO/MD': {
    'id': 'data/SO/MD',
    'isoid': 'MU',
    'key': 'MD',
    'lang': 'so',
    'name': 'Mudug',
  },
  'SO/NG': {
    'id': 'data/SO/NG',
    'isoid': 'NU',
    'key': 'NG',
    'lang': 'so',
    'name': 'Nugaal',
  },
  'SO/SD': {
    'id': 'data/SO/SD',
    'isoid': 'SD',
    'key': 'SD',
    'lang': 'so',
    'name': 'Shabeellaha Dhexe',
  },
  'SO/SG': {
    'id': 'data/SO/SG',
    'isoid': 'SA',
    'key': 'SG',
    'lang': 'so',
    'name': 'Sanaag',
  },
  'SO/SH': {
    'id': 'data/SO/SH',
    'isoid': 'SH',
    'key': 'SH',
    'lang': 'so',
    'name': 'Shabeellaha Hoose',
  },
  'SO/SL': {'id': 'data/SO/SL', 'isoid': 'SO', 'key': 'SL', 'lang': 'so', 'name': 'Sool'},
  'SO/TG': {
    'id': 'data/SO/TG',
    'isoid': 'TO',
    'key': 'TG',
    'lang': 'so',
    'name': 'Togdheer',
  },
  'SO/WG': {
    'id': 'data/SO/WG',
    'isoid': 'WO',
    'key': 'WG',
    'lang': 'so',
    'name': 'Woqooyi Galbeed',
  },
};
